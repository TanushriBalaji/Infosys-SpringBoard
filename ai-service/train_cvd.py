...
print("\n" + "=" * 80)
print("MEDISPHERE - CVD FEDERATED RANDOM FOREST")
print("=" * 80)
...
print(f"CVD positive: {int(y.sum())}")
print(f"No CVD: {int((y == 0).sum())}")
...
print(f"Training CVD: {int(y_train.sum())}")
print(f"Testing CVD: {int(y_test.sum())}")
...
for hospital_number, (_, hospital_idx) in enumerate(
    ...
    print(f"CVD positive: {int(hospital_y.sum())}")
    print(f"No CVD: {int((hospital_y == 0).sum())}")
...
print(f"Total hospital CVD positive: {total_hospital_positive}")
...
model_filename = "cvd_federated_random_forest.joblib"
metrics_filename = "cvd_metrics.json"
